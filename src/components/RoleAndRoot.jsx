...
function RoleAndRoot() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch('http://api.dustipharma.tj:1212/api/v1/app/profile/users', {
          headers: {
        ...
        setUsers(usersList);
        setManagerName(usersList[0]?.Наименование || 'Менеджер не найден');
      } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`http://api.dustipharma.tj:1212/api/v1/app/profile/users/${id}`, {
        method: 'DELETE',
