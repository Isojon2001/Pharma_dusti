...
const fetchUsers = async () => {
  ...
  try {
    ...
    if (!adminResponse.ok || !moderatorResponse.ok) {
    ...
    const adminUsers = Array.isArray(adminData.payload) ? adminData.payload : [];
    const moderatorUsers = Array.isArray(moderatorData.payload) ? moderatorData.payload : [];

    console.log('Admin Users:', adminUsers);
    console.log('Moderator Users:', moderatorUsers);

    const usersList = [...adminUsers, ...moderatorUsers];
  ...
  const handleRoleUpdate = async (userId, newRole) => {  
    try {  
      ...
      setUsers(prev => prev.map(u => (u.id === userId ? { ...u, Роль: newRole } : u)));
      setEditingUser(null);
    } catch (error) {
      ...
      <main className="content">  
        ...
        <div className="back_color_table">  
          <table className="user-table">  
            ...
            <tbody>  
              {paginatedUsers.length > 0 ? paginatedUsers.map((u, index) => (  
                <tr key={u.id || index}>  
